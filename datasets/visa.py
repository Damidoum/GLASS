...
class VisADataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.anomaly_source_paths = sorted(
            1 * glob.glob(anomaly_source_path + "/*/*.jpg")
    ...
    def __getitem__(self, idx):
        ...
        if self.split == DatasetSplit.TRAIN:
            aug = PIL.Image.open(np.random.choice(self.anomaly_source_paths)).convert(
                "RGB"
            )
            if self.rand_aug:
            ...
            aug_image = (
                image * (1 - mask_l) + (1 - beta) * aug * mask_l + beta * image * mask_l
            )
    ...
    def get_image_data(self):
        ...
        for anomaly in anomaly_types:
            relative_img_path = df.loc[
                (df["object"] == self.classname)
                & (df["split"] == flag)
                & (df["label"] == anomaly),
                "image",
            ].values.tolist()
            absolute_img_path = [
                os.path.join(self.source, x) for x in relative_img_path
            ]
            imgpaths_per_class[self.classname][anomaly] = absolute_img_path

            if flag == "test" and anomaly != "normal":
                relative_msk_path = df.loc[
                    (df["object"] == self.classname) & (df["label"] == anomaly), "mask"
                ].values.tolist()
                absolute_msk_path = [
                    os.path.join(self.source, x) for x in relative_msk_path
                ]
